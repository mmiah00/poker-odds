...
function dealFreshRound() {
  ...
  return { hand1: [deck[0], deck[1]], hand2: [deck[2], deck[3]], community: deck.slice(4, 7) };
}
...
function TimerRing({ timeLeft, max }) {
  ...
  return (
    <div style={{ position: 'relative', width: 70, height: 70, flexShrink: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Georgia, serif', fontWeight: 700, fontSize: urgent ? 20 : 18, color, animation: urgent ? 'timerFlash 0.7s ease-in-out infinite' : 'none' }}>
        {timeLeft}
      </div>
    </div>
  );
...
function PlayingCard({ card, size }) {
  ...
  return (
    <div style={{ width: dims.w, height: dims.h, background: '#fdfcf5', borderRadius: 6, border: '1px solid #ddd6c0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', userSelect: 'none', boxShadow: '0 3px 10px rgba(0,0,0,0.55)', flexShrink: 0 }}>
...
function HandPanel({ cards, label, equity, handName, winner, loser, onClick, revealed }) {
  ...
  return (
    <div onClick={!revealed ? onClick : undefined} style={{ cursor: revealed ? 'default' : 'pointer', opacity: loser ? 0.42 : 1, transition: 'transform 0.25s, opacity 0.4s, border-color 0.2s', transform: winner ? 'scale(1.04)' : 'scale(1)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '18px 20px', borderRadius: 13, border: `1.5px solid ${borderCol}`, background: bgCol, minWidth: 160, gap: 11 }}>
      ...
      {revealed ? (
        <div style={{ textAlign: 'center', minHeight: 56, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2 }}>
          <div style={{ color: winner ? '#e8c060' : '#777', fontSize: 26, fontWeight: 700, fontFamily: 'Georgia, serif', lineHeight: 1 }}>{(equity * 100).toFixed(1)}%</div>
...
export default function PokerOddsTrainer() {
  ...
  useEffect(() => {
    if (timeLeft === 0 && !result && !timedOut) { clearInterval(timerRef.current); setTimedOut(true); }
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      setSelected(null); setResult(null); setFeedbackMsg(''); setTimedOut(false); startTimer();
    }, 1400);
  ...
  const handleDeal = useCallback(() => {
    ...
    setSelected(null); setResult(null); setFeedbackMsg(''); startTimer();
  }, [startTimer]);
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse at 50% 0%, #1a3a22 0%, #0c1a10 60%, #060d08 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '28px 16px 48px', fontFamily: 'system-ui, sans-serif' }}>
      <style>{`
      ...
      <div style={{ textAlign: 'center', marginBottom: 22 }}>
        <h1 style={{ fontFamily: 'Georgia, serif', color: '#c9a84c', fontSize: 22, fontWeight: 700, margin: 0, letterSpacing: 3 }}>POKER ODDS TRAINER</h1>
        <div style={{ color: '#4a7a5a', fontSize: 10, marginTop: 5, letterSpacing: 2 }}>POST-FLOP · WHICH HAND HAS BETTER ODDS?</div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 22 }}>
        <div style={{ display: 'flex', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 9, overflow: 'hidden' }}>
          {[{ label: 'ACCURACY', value: accuracy === '--' ? '—' : `${accuracy}%` }, { label: 'ROUNDS', value: stats.total }, { label: 'STREAK', value: stats.streak, hot: stats.streak >= 3 }].map((s, i) => (
      ...
      {timedOut && <div style={{ marginBottom: 12, color: '#d04040', fontSize: 14, fontWeight: 600, animation: 'fadeUp 0.3s ease' }}>Time's up! Dealing new hands...</div>}

      <div style={{ marginBottom: 20, textAlign: 'center' }}>
        <div style={{ color: '#3a5a44', fontSize: 9, letterSpacing: 3, marginBottom: 8, fontFamily: 'Georgia, serif' }}>THE FLOP</div>
      ...
      <div style={{ display: 'flex', gap: 14, marginBottom: 20, flexWrap: 'wrap', justifyContent: 'center', opacity: timedOut ? 0.3 : 1, transition: 'opacity 0.4s' }}>
        <HandPanel cards={hand1} label="HAND A" equity={result?.equity1} handName={result?.hand1Name} winner={result?.winner === 1} loser={result !== null && result.winner === 2} selected={selected === 1} onClick={() => handlePick(1)} revealed={result !== null} />
        <div style={{ display: 'flex', alignItems: 'center', color: '#2a4a34', fontSize: 15, fontWeight: 700 }}>VS</div>
        <HandPanel cards={hand2} label="HAND B" equity={result?.equity2} handName={result?.hand2Name} winner={result?.winner === 2} loser={result !== null && result.winner === 1} selected={selected === 2} onClick={() => handlePick(2)} revealed={result !== null} />
      </div>
      ...
      {result && (
        <div style={{ width: '100%', maxWidth: 380, textAlign: 'center', animation: 'fadeUp 0.35s ease' }}>
          ...
          <div style={{ marginTop: 14, color: result.winner === 'tie' ? '#c9a84c' : isCorrect ? '#50c878' : '#c05050', fontSize: 15, fontWeight: 600 }}>
            {isCorrect && result.winner !== 'tie' && '✓ '}{!isCorrect && result.winner !== 'tie' && '✗ '}{feedbackMsg}
          </div>
          {result.tieRate > 0.005 && <div style={{ color: '#3a5a44', fontSize: 10, marginTop: 5 }}>Ties {(result.tieRate * 100).toFixed(1)}% of runouts</div>}
      ...
      {!result && !timedOut && <div style={{ color: '#2a4a34', fontSize: 11, letterSpacing: 1, marginTop: 4 }}>Click a hand to reveal the odds</div>}
    </div>
  );
}
